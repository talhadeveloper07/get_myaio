...
<div class='container-fluid main-sec bg-white'>
      ...
      <div class="col-md-6 p-0 py-4">
        <div class="form-col">
          <h2 class="text-center mt-3" style='font-weight:bold;'>Business Information</h2>  
          ...
          <input id="search" class="form-control" type="text" placeholder="Search Your Business Here"/> 
           <form method='post' action="{{ route('add-radius')}}">
           @csrf
                    <div class="row">
                      <div class="col-md-6">
                      ...
                      <div class="col-md-6">
                        <div class="form-group my-3">
                        <input type="email" class="form-control" id="bemail" name='bemail' placeholder="Enter Business Email">
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="bphone" name='bphone' placeholder="Enter Business Phone">
                          </div>
                      </div>
                       <div class="col-md-6">
                          <div class="form-group">   
                        <input name='website_url' id='website' class='form-control' placeholder='Enter website URL'>
                          </div>
                      </div>
                    </div>
                   
                    <div class="form-group my-3">    
                        <textarea name='baddress' id='baddress' class='form-control' rows='2' placeholder='Enter Business Address'></textarea>
                    </div>
                    <div class="form-group my-3">
...
function initAutocomplete() {
  ...
  markers.push(marker);
    ...
    function initAutocomplete() {
  const map = new google.maps.Map(document.getElementById("map"), {
    center: { lat: 37.0902, lng: -95.7129 }, // Centered at the United States
    zoom: 4,
  });
  ...
  searchBox.addListener("places_changed", () => {
    ...
    // Check if the selected place is in the United States or Canada
    const selectedPlace = places[0];
    const countryRestrictions = ["US", "CA"]; // United States and Canada

    if (!selectedPlace || !selectedPlace.address_components) {
      alert("Invalid place or address.");
      return;
    }

    const placeCountry = selectedPlace.address_components.find(
      (component) => component.types.includes("country")
    );

    if (placeCountry && countryRestrictions.includes(placeCountry.short_name)) {
      // Clear out the old markers.
      markers.forEach((marker) => {
        marker.setMap(null);
      });
    ...
    placeDetails.getDetails({ placeId: selectedPlace.place_id }, (place, status) => {
      ...
      alert("Please select a place in the United States or Canada.");
    }
  });
}
